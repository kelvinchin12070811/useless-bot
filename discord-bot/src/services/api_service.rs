...
pub async fn db_auth(
    config_context: &ConfigContext,
    auth_token_context: Arc<Mutex<APIAuthTokenContext>>,
) {
    ...
    {
        let mut auth_token_context = auth_token_context.lock().unwrap();
...
pub async fn get_auth_token(
    ...
    let is_token_expired = {
        let auth_token = auth_token_context.lock().unwrap();
        auth_token.get_expired_date() <= &Utc::now()
    };

    if is_token_expired {
        db_auth(&config_context, auth_token_context.clone()).await;
    }

    let token = {
        let auth_token = auth_token_context.lock().unwrap();
        auth_token.get_token().to_string()
    };
    token
}
